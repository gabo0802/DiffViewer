...
use crate::diff_engine::{unified_parser, twoway};
use crate::io;

/// Import a unified diff / patch file into the current workspace.
pub fn import_patch(conn: &Connection, workspace_id: &str, patch_path: &str) -> Result<String, String> {
    ...
    store::insert_diffset(conn, &DiffSet {
    ...
    for pf in &parsed {
        ...
        store::insert_filediff(conn, &FileDiff {
            filediff_id,
            diffset_id: diffset_id.clone(),
            display_path: pf.new_path.clone(),
...
pub fn compare_two_files(
    ...
    let now = chrono::Utc::now().timestamp();
    store::insert_snapshot(conn, &Snapshot { snapshot_id: ls_id.clone(), sha256: ls_hash, size_bytes: ls_size, cache_path: ls_cache, created_at: now })?;
    store::insert_snapshot(conn, &Snapshot { snapshot_id: rs_id.clone(), sha256: rs_hash, size_bytes: rs_size, cache_path: rs_cache, created_at: now })?;
    ...
    let filediff_id = uuid::Uuid::new_v4().to_string();
    let left_name = left_resolved.file_name().map(|n| n.to_string_lossy().into_owned()).unwrap_or_default();
    let right_name = right_resolved.file_name().map(|n| n.to_string_lossy().into_owned()).unwrap_or_default();
    ...
    store::insert_filediff(conn, &FileDiff {
        ...
        content_left_json: serde_json::json!({ "type": "snapshot", "snapshot_id": ls_id }).to_string(),
        content_right_json: serde_json::json!({ "type": "snapshot", "snapshot_id": rs_id }).to_string(),
        hunks_json,
        write_target_json: serde_json::json!({ "type": "path", "path": right_resolved.to_string_lossy() }).to_string(),
        created_at: now,
    })?;
